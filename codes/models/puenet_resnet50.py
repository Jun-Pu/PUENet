...
resnet_pretrain = resnet50(pretrained=True)
...
class PUAModule(nn.Module):
    def __init__(self, ndf):
        ...
        self.conv1 = nn.Conv2d(4, ndf, kernel_size=3, stride=2, padding=1)  # 4 for predictive uncertainty
...
class BCVAEModule(BaseModel):
    ...
    def initialize_weights(self):
        pretrained_dict = resnet_pretrain.state_dict()
        all_params = {}
        for k, v in self.encoderRes50.state_dict().items():
            if k in pretrained_dict.keys():
                v = pretrained_dict[k]
                all_params[k] = v
        self.encoderRes50.load_state_dict(all_params)
    ...
    def forward(self, x, y=None):
        raw_x = x
        EnFeat_4, EnFeat_3, EnFeat_2, EnFeat_1 = self.encoderRes50.forward(x)
