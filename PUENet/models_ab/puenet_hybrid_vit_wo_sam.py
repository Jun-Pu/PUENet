...
class InferenceModel_x(nn.Module):
    def __init__(self, input_channels, channels, latent_size):
        ...
        self.fc1 = nn.Linear(channels * 8 * 16 * 16, latent_size)  # adjust according to input size
        self.fc2 = nn.Linear(channels * 8 * 16 * 16, latent_size)  # adjust according to input size
    ...
    def forward(self, input):
        ...
        output = output.view(-1, self.channel * 8 * 16 * 16)  # adjust according to input size
        # print(output.size())
...
class BCVAEModule(BaseModel):
    def __init__(self,
        ...
        self.RFB_L4 = self._make_pred_layer(Classifier_Module, [3, 6, 12, 18], [3, 6, 12, 18], channel, 768)
        self.RFB_L3 = self._make_pred_layer(Classifier_Module, [3, 6, 12, 18], [3, 6, 12, 18], channel, 768)
    ...
    def forward(self, x, y=None):
        raw_x = x
        EnFeat_1, EnFeat_2, EnFeat_3, EnFeat_4 = forward_vit(self.pretrained, x)
        ...
        if y == None:
            ...
            D_out_prior = self.decoder_prior(D4, D3, D2, D1, z_prior)
            ...
            D_out_prior = self.decoder_prior(D4, D3, D2, D1, z_prior)
            D_out_post = self.decoder_post(D4, D3, D2, D1, z_post)
...
class BCVAEModule_decoder(BaseModel):
    def __init__(self,
        ...
        self.CAtten4 = CALayer(channel)
        self.CAtten3 = CALayer(channel)
        self.CAtten2 = CALayer(channel)
        self.CAtten1 = CALayer(channel)
    ...
    def forward(self, D4, D3, D2, D1, z):
